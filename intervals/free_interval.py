...
def free_interval(busy: list[list[int]], work_start: int, work_end:int) -> list[list[int]]:
    free = []
    busy.sort()
 
    for start, end in busy:
        if start > work_start:
            free.append([work_start, start])
        work_start = end
...
busy=[[9,10],[12,14],[16,17]]
free_interval(busy, 9, 17)